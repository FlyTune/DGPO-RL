...
def compute_policy_loss(
    ...
    assert ratio_clip_config.ratio_mode in ["ppo", "dgpo"], "Only PPO and DGPO are supported for now."
    ...
    elif ratio_clip_config.ratio_mode == "dgpo":
        dgpo_n = ratio_clip_config.dgpo_n
        dgpo_m = ratio_clip_config.dgpo_m
        ratio = torch.where(
            negative_lower_bound_case,
            torch.exp(log_prob + dgpo_n * log_prob.detach() - (dgpo_n+1) * old_log_prob) / (lower_bound ** dgpo_n),
            ratio
        )
        ratio = torch.where(
            positive_upper_bound_case,
            torch.exp(log_prob + (log_upper_bound / dgpo_m) - old_log_prob * (dgpo_m-1) / dgpo_m - log_prob.detach() / dgpo_m),
            ratio
        )
